...
public:
  ...
  const point2D &operator=(const point2D<T> &other) {
  ...
  T norm() const { return _x * _x + _y * _y; };
  ...
  point2D conjugate() const { return point2D(_x, -_y); }
  point2D negate() const { return point2D(-_x, -_y); }
...
template <typename T> istream &operator>>(istream &in, point2D<T> &other) {
  in >> other.x() >> other.y();
  return in;
}
...
template <typename T> T cross(const point2D<T> &p1, const point2D<T> &p2) {
...
template <typename T> int sigcross(point2D<T> p1, point2D<T> p2) {
...
template <typename T1, typename T2> T1 dot(point2D<T1> p1, point2D<T2> p2) {
  return p1.x() * p2.x() + p1.y() * p2.y();
}
template <typename T> T norm(const point2D<T> &p) { return p.norm(); }

template <typename T> double dnorm(const point2D<T> &p) {
  return 1.0 * p._x * p._x + 1.0 * p._y * p._y;
}
template <typename T> double length(const point2D<T> &p) {
  return sqrt(dnorm(p));
}
//*********************************************************
// 3D point
...
public:
  ...
  point3D(const T xx = T(), const T &yy = T(), const T &zz = T())
      : _x(xx), _y(yy), _z(zz){};
  ...
  double dnorm() const { return 1.0 * _x * _x + 1.0 * _y * _y + 1.0 * _z * _z; }
  double length() const { return sqrt(dnorm()); };
};
template <typename T1, typename T2>
...
template <typename T1, typename T2> T1 dot(point3D<T1> p1, point3D<T2> p2) {
...
template <typename T> istream &operator>>(istream &in, point3D<T> &other) {
  in >> other.x() >> other.y() >> other.z();
  return in;
}

template <typename T> T norm(const point3D<T> &p) { return p.norm(); }

template <typename T> double dnorm(const point3D<T> &p) { return p.dnorm(); }
template <typename T> double length(const point3D<T> &p) { return p.length(); }
// sort vectors clockwise
template <typename T> int get_type(const point2D<T> &a) {
  if (a.y())
    return a.y() > 0 ? 1 : -1;
  assert(a.x());
  return a.x() > 0 ? 1 : -1;
...
template <typename T>
point3D<double> point_plane_intersection(point3D<T> p0, point3D<T> px,
                                         point3D<T> pv) {
  double t = ddot(p0 - px, pv) / dnorm(pv);
  return point3D<double>(p0.x() + t * pv.x(), p0.y() + t * pv.y(),
                         p0.z() + t * pv.z());
}
...
bool linesIntersection(point2D<T> p1, point2D<T> p2, point2D<T> p3,
  ...
  if (fabs(dcross(p1 - p2, p3 - p4)) < EPS)
    return false;
...
bool linesIntersection(double kx1, double ky1, double k1, double kx2,
                       double ky2, double k2, point2D<double> &p) {
  double t;
  if (fabs(t = kx1 * ky2 - kx2 * ky1) < EPS)
    return false;
...
template <typename T>
int segmentsIntersection(point2D<T> p1, point2D<T> p2, point2D<T> p3,
                         point2D<T> p4, point2D<double> &inter) {
  if (sigcross(p3 - p1, p2 - p1) * sigcross(p2 - p1, p4 - p1) < 0)
    return 0;
  if (sigcross(p1 - p3, p4 - p3) * sigcross(p4 - p3, p2 - p3) < 0)
    return 0;
  ...
  if (!linesIntersection(p1, p2, p3, p4, inter))
    *((int *)0) = 0;
  return 1;
}
...
template <typename T>
double pointLineDistance(point2D<T> p, point2D<T> p0, point2D<T> dp,
                         double &tt) {
  tt = 0.0;
  double a = dp.dnorm();
...
template <typename T>
int lineSegmentIntersection(point2D<T> p1, point2D<T> p2, double a, double b,
                            double c, dpoint2D &inter) {
  double de = 1.0 * (p2.x() - p1.x()) * a + 1.0 * (p2.y() - p1.y()) * b;
  double no = -c - a * p1.x() - b * p1.y();
  if (fabs(de) < EPS)
    return fabs(no) < EPS ? 2 : 0;
  double t = no / de;
  if (t < -EPS || t > 1.0 + EPS)
    return 0;
...
// Find one intersection of two circles. Return the upper intersection point if
// x1<x2
template <typename T>
bool circlesIntersection(point2D<T> p1, T r1, point2D<T> p2, T r2,
                         point2D<double> &inter) {
  double dx = p2.x() - p1.x();
  double dy = p2.y() - p1.y();
  double d = mysqrt(dx * dx + dy * dy);
  if (d > r1 + r2 + EPS || d + EPS < fabs(r1 - r2))
    return false;
  // circle overlap
  if (d < EPS)
    return false;
  ...
  inter = point2D<double>(1.0 * p1.x() + (e * dx - f * dy) / d,
                          1.0 * p1.y() + (f * dx + e * dy) / d);
  return true;
}
...
template <typename T>
bool lineCircleIntersection(point2D<T> p0, point2D<T> dp, point2D<T> center,
                            T r, double &t1, double &t2) {
  if (dp.dnorm() < EPS) {
    t1 = t2 = 0.0;
...
// Return false if there are an infinite number of such circles or no such
// circle
bool circle_on_point3(dpoint2D a, dpoint2D b, dpoint2D c, dpoint2D &center,
                      double &radius) {
  ...
  if (!linesIntersection(ab0, ab1, ac0, ac1, center))
    return false;
...
double circleOverlap(point2D<T> c1, double r1, point2D<T> c2, double r2) {
  if (length(c1 - c2) + EPS > r1 + r2)
    return 0.0;
  if (r1 > r2)
    swap(c1, c2), swap(r1, r2);
  if (length(c1 - c2) < r2 - r1 + EPS)
    return PI * r1 * r1;
...
vector<point2D<T>> convexHull(vector<point2D<T>> all, bool nocolinear = true) {
  ...
  for (int i = 1; i < all.size(); i++) {
    // If all[qq.back()]==all[qq[qq.size()-2]], then all point2D<T> s will be
    // pushed to the stack
    while (qq.size() >= 2 &&
           llcross(all[i] - all[qq.back()],
  ...
  for (int i = 1; i + 1 < qq.size(); i++)
    res.push_back(all[qq[i]]);
  return res;
}
...
vector<point2D<double>> cut_convex(const vector<point2D<double>> &convex,
  ...
  for (int i = 0, pre = 0, next = 0; i < n; i++) {
    ...
    if (inside[pre] != inside[next]) {
      bool cnt =
          linesIntersection(start, end, convex[pre], convex[next], inter);
      assert(cnt);
      ret.push_back(inter);
    }
    if (inside[next])
      ret.push_back(convex[next]);
    pre = next;
  }
  return ret;
}
template <typename T> T convex_area(const vector<point2D<T>> &convex) {
  const int n = convex.size();
  T ret = 0;
  for (int i = 0; i < n; i++)
    ret += cross(convex[i], convex[(i + 1) % n]);
  if (ret < 0)
    ret = -ret;
  ret /= 2;
  return ret;
...
struct rect_t {
  ...
  bool operator<(const rect_t &other) const { return y1 < other.y1; }
  ...
  rect_t intersaction(const rect_t &other) {
    rect_t res(max(x1, other.x1), min(x2, other.x2), max(y1, other.y1),
               min(y2, other.y2));
    if (res.x1 > res.x2)
      res.x1 = res.x2;
    if (res.y1 > res.y2)
  ...
  int Xdim() { return x2 - x1; }
  int Ydim() { return y2 - y1; }
};
template <typename T> class CPoint : public complex<T> {
public:
  CPoint(const T &x = T(), const T &y = T()) : complex<T>(x, y){};
  T &x() { return complex<T>::real(); }
  T &y() { return complex<T>::imag(); }
};
