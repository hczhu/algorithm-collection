...
public:
  ...
  Huffman_encode_table_t()
      : capacity(0), n(0), code(NULL), length(NULL), map_table(NULL){};
...
public:
  ...
  Huffman_decode_table_t()
      : capacity(0), n(0), offset(NULL), map_table(NULL), first_code(NULL),
        min_code_length(0){};
  int create(const Huffman_encode_table_t &encode_table);
  int Huffman_decode_nitem(const uint32_t[], const uint64, uint64 &, int[],
                           int) const;
  ~Huffman_decode_table_t();
};
};
...
int Huffman_encode_table_t::resize(int size) {
  ...
  try {
    ...
    Delete_array(map_table);
    Huffman_encode_table_t();
    com_writelog(COMLOG_FATAL,
                 "Alolcating memory for Huffman_encode_table_t failed");
    return -1;
  }
  return 0;
}
int Huffman_encode_table_t::get_code(int idx, uint &out_code,
                                     uint &out_len) const {
  if (idx < 0 || idx >= n) {
    com_writelog(COMLOG_FATAL, "[idx:%d] does not in the range [%d,%d] in "
                               "Huffman_encode_table_t::get_code",
                 idx, 0, n - 1);
    return -1;
  }
  out_code = code[idx];
...
int Huffman_decode_table_t::resize(int size) {
  ...
  try {
    ...
    Huffman_decode_table_t();
    com_writelog(COMLOG_FATAL,
                 "Alolcating memory for Huffman_decode_table_t failed");
    return -1;
  }
  return 0;
}
...
*/
void counting_sort(uint count_array[], uint32_t count_size, uint item[],
                   uint32_t value[], int n, uint buffer[]) {
...
int Huffman_encode_table_t::create(const uint frequency[], int fn,
                                   const int map_t[]) {
  if (fn <= 0) {
  ...
  uint *buf1 = NULL, *buf2 = NULL, *buf3 = NULL, *buf4 = NULL,
       *count_array = NULL;
  try {
    ...
    com_writelog(
        COMLOG_FATAL,
        "Allocating buffers for Huffman_encode_table_t::create failed");
    return -1;
  }
  uint *item = buf1;
  uint *value = buf2;
  ...
  while (tail1 - head1 + n - head > 1) {
    ...
    if (head1 == tail1)
      ...
      a = item[head1++];
    else if (value[item[head]] <= value[item[head1]])
      a = item[head++];
    else
      a = item[head1++];
  ...
  //  std::cerr<<item[n-1]<<":"<<map_table[item[n-1]]<<" "<<code[item[n-1]]<<"
  //  "<<length[item[n-1]]<<std::endl;
  ...
  for (int i = n - 2; i >= 0; i--) {
    ...
    if (length[item[i]] == length[item[i + 1]]) {
      ...
      code[item[i]] =
          (code[item[i + 1]] + 1) >> (length[item[i + 1]] - length[item[i]]);
    }
    assert(code[item[i]] < n);
    assert(length[item[i]] < n);
    assert(length[item[i]] >= 32 || code[item[i]] < TWO(length[item[i]]));
    //    std::cerr<<item[i]<<":"<<map_table[item[i]]<<" "<<code[item[i]]<<"
    //    "<<length[item[i]]<<std::endl;
  }
  delete[] count_array;
  delete[] buf1;
  delete[] buf2;
...
int Huffman_decode_table_t::create(const Huffman_encode_table_t &encode_table) {
  ...
  if (NULL == (count_array)) {
    com_writelog(COMLOG_FATAL,
                 "Allocating buffer for Huffman_decode_table_t::create");
    return -1;
  }
  ...
  for (int i = 0; i < n; i++) {
    uint len = length[i];
    map_table[offset[len] + code[i] - first_code[len]] =
        encode_table.map_table[i];
  }
#ifndef NDEBUG
//  puts("map_table");
...
int Huffman_decode_table_t::serialization(FILE *out_file) const {
  if (1 != fwrite(&n, sizeof(n), 1, out_file) ||
    ...
    com_writelog(COMLOG_FATAL,
                 "Huffman_decode_table_t::serialization writing file failed");
    return -1;
  }
  return 0;
}
int Huffman_decode_table_t::deserialization(FILE *in_file) {
  ...
  if (0 > resize(n)) {
    com_writelog(COMLOG_FATAL,
                 "resize in Huffman_decode_table_t::deserialization failed");
    return -1;
  }
...
int Huffman_decode_table_t::Huffman_decode_nitem(const uint32_t bit_stream[],
  ...
  while (res < maximum_items) {
    ...
    if (bit_offset > bit_length) {
      com_writelog(
          COMLOG_FATAL,
          "code boundary exceeds [bit_length:%llu] [offset%llu] [code%u]",
          bit_length, bit_offset, code);
      return -1;
    }
    if (code >= (uint)n) {
      com_writelog(COMLOG_FATAL, "Illegal Huffman code [code:%u] [n:%d]", code,
                   n);
      return -1;
    }
