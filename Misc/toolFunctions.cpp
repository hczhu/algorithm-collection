...
#define GOOD(grid, r, c)                                                       \
  (((r) >= 0 && (r) < grid.size()) && ((c) >= 0 && (c) < grid[0].length()))
#define BCNT(x) __builtin_popcount(x)
#define Min(out, ...)                                                          \
  do {                                                                         \
    typeof(out) X[] = {__VA_ARGS__};                                           \
    out = *min_element(X, X + sizeof(X) / sizeof(X[0]));                       \
  } while (0)
#define Max(out, ...)                                                          \
  do {                                                                         \
    typeof(out) X[] = {__VA_ARGS__};                                           \
    out = *max_element(X, X + sizeof(X) / sizeof(X[0]));                       \
  } while (0)
#define uMin(out, ...)                                                         \
  do {                                                                         \
    typeof(out) X[] = {out, __VA_ARGS__};                                      \
    out = *min_element(X, X + sizeof(X) / sizeof(X[0]));                       \
  } while (0)
#define uMax(out, ...)                                                         \
  do {                                                                         \
...
#define Foreach(itr, con)                                                      \
  typedef typeof(con) _type_of_##con;                                          \
  for (_type_of_##con::iterator itr = con.begin(); itr != con.end(); ++itr)
#define Make_vector(vec, ...)                                                  \
  do {                                                                         \
    typedef typeof(vec) _T;                                                    \
    const _T::value_type _tmp[] = {__VA_ARGS__};                               \
    vec = _T(_tmp, _tmp + sizeof(_tmp) / sizeof(_tmp[0]));                     \
  } while (0)
#define Union(out, ...)                                                        \
  do {                                                                         \
    typeof(out) _v;                                                            \
    Make_vector(_v, __VA_ARGS__);                                              \
    out = interval_union(_v);                                                  \
  } while (0)
...
template <class T> string convert(vector<T> vv) {
  ostringstream re;
...
template <> vector<string> parse(const string &ss, const char *cut) {
  vector<string> re;
  for (int j = 0; j < ss.size(); j++) {
    string s;
    while (j < ss.size() && NULL == strchr(cut, ss[j]))
...
template <typename T> vector<T> set_diff(vector<T> &a, vector<T> &b) {
  vector<T> c;
  sort(a.begin(), a.end());
  sort(b.begin(), b.end());
...
template <typename T> bool next_permutation(T ptr[], int n) {
  for (int i = n - 1; i; i--)
    if (ptr[i] > ptr[i - 1]) {
      int b = i;
      while (b + 1 < n && ptr[b + 1] > ptr[i - 1])
        b++;
      swap(ptr[i - 1], ptr[b]);
      reverse(ptr + i, ptr + n);
      return true;
...
vector<vector<int>> generateCubePermutation() {
  ...
  set<vector<int>> mem;
  const int rot[][4] = {{2, 3, 4, 5}, {0, 2, 1, 4}, {0, 3, 1, 5}};
  for (int a = 0; a < 4; a++)
    for (int b = 0; b < 4; b++)
      for (int c = 0; c < 4; c++) {
        // a=0;b=1;c=0;
