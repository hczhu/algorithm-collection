...
struct Hnode
{
  ...
  Hnode(int kk):key(kk),rank(0),son(NULL),pre(NULL),next(NULL),pp(NULL),mark(false){};
  Hnode():rank(0),son(NULL),pre(NULL),next(NULL),pp(NULL),mark(false){};
};
class FibHeap
{
  private:
    int kcmp(const Hnode* p1,const Hnode* p2)
    {
      return hcmp(p1->key,p2->key);
    }
    Hnode *root;
    int (*hcmp)(int,int);
  ...
  public:
    ...
    int getMin(){assert(root!=NULL);return root->key;};
...
void FibHeap::doubleLink(Hnode* ph,Hnode* pn)
{
  ...
  assert(pn);
  Hnode* next=ph->next;
  ph->next=pn;pn->pre=ph;
  pn->next=next;next->pre=pn;
}
...
void FibHeap::merge(FibHeap& fh)
{
  ...
  else if(fh.root)
  {
    ...
    root->next=n2;n2->pre=root;
    fh.root->next=n1;n1->pre=fh.root;
    if(0>kcmp(root,fh.root))root=fh.root;
  }
  fh.root=NULL;
}
...
Hnode* FibHeap::mergeTree(Hnode* r1,Hnode* r2)
{
  ...
  if(kcmp(r1,r2)>0)std::swap(r1,r2);
  r1->rank++;
  if(r1->son==NULL)r1->son=r2,r2->pre=r2->next=r2;
  else doubleLink(r1->son,r2);
  r2->pp=r1;
  return r1;
}
...
void FibHeap::decrease(Hnode* ph)
{
  ...
  while(pp->mark)
  {
    ...
    cutHnode(pp);
    doubleLink(root,pp);
    if(0<kcmp(root,pp))root=pp;
    pp=ph;
  }
  assert(pp);
  if(pp->pp)pp->mark=true;
}