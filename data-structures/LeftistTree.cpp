#include <algorithm>
#include <assert.h>
template <int S> class LeftistTree {
  int ll[S + 1], rr[S + 1], size[S + 1], key[S + 1];
  int top;
  int (*kcmp)(int, int);

public:
  LeftistTree(int (*cmp)(int, int)) : top(0), kcmp(cmp) {
    size[0] = ll[0] = rr[0] = 0;
  };
  int makeTree(int kk) {
    top++;
