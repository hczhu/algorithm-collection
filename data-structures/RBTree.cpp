...
template<typename Key,class Comp=default_cmp_t<Key> >
class RBTree
{    
public:
	struct RBNode
...
private:
	...
	int nodeCmp(RBNode* a,RBNode* b){return a==b?0:kcmp(a->key,b->key);};
#define OUTPUT
#ifdef OUTPUT
    ...
    public:
		...
		void clear(){root=null;null->pp=null;};
		...
		RBNode* getRoot(RBNode* p){while(p->pp!=null)p=p->pp;return p;};
		bool empty(){return root==null;};
		int count_not_greater_than(RBNode& node);
		void setRoot(RBNode* pr)
		{
			root=pr;root->pp=null;
			if(root->red)
...
int RBTree<Key,Comp>::DFS(RBNode* v,bool f=false)
{
	...
	if(v==null)return 1;
#ifdef SIZE
	if(f)printf("Node%d:key=%d ll=%d rr=%d pp=%d size=%d red=%s bh=%d\n",
		v-null,v->key,v->ll-null,v->rr-null,v->pp-null,v->size,v->red?"red":"black",v->bh);
#else
	if(f)printf("Node%d:key=%d ll=%d rr=%d pp=%d red=%s bh=%d\n",
		v-null,v->key,v->ll-null,v->rr-null,v->pp-null,v->red?"red":"black",v->bh);
#endif
	assert(v->ll==null||v->ll->pp==v);
	assert(v->rr==null||v->rr->pp==v);
	int h=DFS(v->ll,f);
	if(v->red)assert(!(v->ll->red)&&!(v->rr->red));
	assert(h==DFS(v->rr,f));
	if(!(v->red))h++;
	assert(h==v->bh);
#ifdef SIZE
...
template<typename Key,class Comp>
void RBTree<Key,Comp>::printTree()
{
	puts("----------------------");
	DFS(root,true);
}
...
typename RBTree<Key,Comp>::RBNode* RBTree<Key,Comp>::lowerBound(Key k)
{
	...
	while(re!=null&&kcmp(re->key,k)<0)
		re=re->rr;
	if(re==null)return end();
	RBNode* v=re->ll;
	while(v!=null)
	{
		if(kcmp(v->key,k)<0)v=v->rr;
		else re=v,v=v->ll;
	}
...
typename RBTree<Key,Comp>::RBNode* RBTree<Key,Comp>::kelement(int k)
{
	...
	for(;;)
	{
		assert(v!=null&&v);
		assert(k);
		if(v->ll->size+1==k)return v;
		if(v->ll->size>=k)v=v->ll;
		else k-=v->ll->size+1,v=v->rr;
	}
	return 0;
}
template<typename Key,class Comp>
...
void RBTree<Key,Comp>::leftRotate(RBNode* x)
{
    ...
    y->rr=x->ll;y->rr->pp=y;
	x->ll=y;y->pp=x;x->pp=z;
    if(z==null)root=x;
    else if(z->ll==y)z->ll=x;
    else if(z->rr==y)z->rr=x;
    else assert(0);
#ifdef SIZE
...
template<typename Key,class Comp>
void RBTree<Key,Comp>::insert(RBNode* z)
{
	assert(z!=root);
	z->ll=z->rr=z->pp=null;
#ifdef SIZE
...
#ifdef SIZE
	for(RBNode* p=z->pp;p!=null;p=p->pp)p->size++;
#endif
	insertFixup(z);
}
template<typename Key,class Comp>
...
void RBTree<Key,Comp>::insertFixup(RBNode* z)
{
    while(z->pp->red)
    {
        ...
        if(z->pp->pp->ll==z->pp)
        {
            ...
            {
                if(z->pp->rr==z)leftRotate(z),z=z->ll;
                assert(z->pp->pp->rr==y);
                assert(z==z->pp->ll);
                rightRotate(z->pp);
				assert(!z->pp->rr->red);
        ...
        else
        {
            ...
            {
                if(z->pp->ll==z)rightRotate(z),z=z->rr;
                assert(z->pp->pp->ll==y);
                assert(z==z->pp->rr);
                leftRotate(z->pp);
//				cerr<<z<<" "<<z->pp<<" "<<z->pp->ll<<" "<<root<<" "<<null<<endl;
				assert(!z->pp->ll->red);
                z->pp->ll->red=true;
				z->pp->ll->bh--;
				z->pp->red=false;
...
template<typename Key,class Comp>
void RBTree<Key,Comp>::remove(RBNode* x)
{
    assert(x&&x!=null);
    if(x->ll!=null&&x->rr!=null)
    {
        ...
        RBNode* z=y->ll;
        z->pp=y->pp;
        if(z->pp->ll==y)z->pp->ll=z;
        else z->pp->rr=z;
#ifdef SIZE
		for(RBNode* p=y->pp;p!=null;p=p->pp)p->size--;
#endif
		y->pp=x->pp;y->ll=x->ll;y->rr=x->rr;
#ifdef SIZE
		y->size=x->size;
#endif
		y->bh=x->bh;
        if(root==x)root=y;
        if(y->pp->ll==x)y->pp->ll=y;
        else y->pp->rr=y;
        y->ll->pp=y;y->rr->pp=y;
        if(!y->red)
        {
            y->red=x->red;
            if(z==null)assert(z->pp->rr!=null||z->pp->ll!=null);
            removeFixup(z);
        }
        else y->red=x->red;
    }
    else
    {
        ...
        if(root==null)return;
#ifdef SIZE
		for(RBNode* p=x->pp;p!=null;p=p->pp)p->size--;
#endif
        z->pp=x->pp;
        if(z->pp->ll==x)z->pp->ll=z;
        else z->pp->rr=z;
        if(!(x->red))
        {
            if(z==null)assert(z->pp->ll!=null||z->pp->rr!=null);
            removeFixup(z);
        }
    }
}
...
void RBTree<Key,Comp>::removeFixup(RBNode* x)
{
    while(x!=root&&!x->red)
    {
        if(x->pp->ll==x)
        {
            ...
            leftRotate(w);
			w->red=w->ll->red;w->bh=w->ll->bh;
			w->ll->red=false;w->ll->bh=x->bh+1;
			w->rr->red=false;w->rr->bh++;
            x=root;
        }
        else
        {
            ...
            rightRotate(w);
			w->red=w->rr->red;w->bh=w->rr->bh;
			w->rr->red=false;w->rr->bh=x->bh+1;
			w->ll->red=false;w->ll->bh++;
            x=root;
        }
    }
    x->red=false;
...
typename RBTree<Key,Comp>::RBNode* RBTree<Key,Comp>::predecessor(RBNode* x)
{
	...
	if(x->ll==null&&x!=root)
    {
		while(x!=root&&x->pp->ll==x)x=x->pp;
        if(x==root)return NULL;
        return x->pp;
    }
    x=x->ll;
	if(x==null)return NULL;
    while(x->rr!=null)x=x->rr;
    return x; 
}
template<typename Key,class Comp>
typename RBTree<Key,Comp>::RBNode* RBTree<Key,Comp>::successor(RBNode* x)
{
	...
	if(x->rr==null&&x!=root)
    {
		while(x!=root&&x->pp->rr==x)x=x->pp;
        if(x==root)return NULL;
        return x->pp;
    }
    x=x->rr;
	if(x==null)return NULL;
    while(x->ll!=null)x=x->ll;
    return x;
}
template<typename Key,class Comp>
...
void RBTree<Key,Comp>::split(RBTree& lt,RBNode *pm,RBTree& rt)
{
	...
	lt.setRoot(pm->ll);rt.setRoot(pm->rr);
	pm->ll=pm->rr=NULL;
	RBNode* ppm;
	int h=root->bh*2;
...
{
	std::set<int> mem;
	for(int i=0;i<1000000;i++)mem.insert(rand());
	for(int i=0;i<10000000;i++)mem.find(rand());
/*	RBTree<int> tree;
