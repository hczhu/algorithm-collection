...
The algorithm first delete some columns to reduce the number of column to not
greater than the number of rows,
using the following two facts:
1. If A[a][a]<A[a][a+1], then for any 0<=i<=a A[i][a]<A[i][a+1], so the elements
A[i][a+1]s are all "dead".
2. If A[a][a]>=A[a][a+1], then for any a<=i<n A[i][a]>=A[i][a+1], so the
elements A[i][a]s are all "dead".
Then it partitions the row into two parts even rows and odd rows, and
recursively calculates the row minima of even rows.
*/
#include <iostream>
#include <vector>
//#define NDEBUG
