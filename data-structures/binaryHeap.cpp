...
using namespace std;
// Heap
template <bool (*cmpLess)(int, int)> class binaryHeap {
  vector<int> heap, hpos;
  int hs;

public:
  binaryHeap<cmpLess>(int cap = 100) {
    hs = 0;
    heap.resize(cap);
    hpos.resize(cap);
  };
  ...
  void adjustDown(int key) {
    ...
    while ((pos << 1) + 1 < hs) {
      mm = 1 + (pos << 1);
      if (mm + 1 < hs && cmpLess(heap[mm + 1], heap[mm]))
        mm++;
      if (cmpLess(key, heap[mm]))
        break;
      heap[pos] = heap[mm];
      hpos[heap[pos]] = pos;
      pos = mm;
