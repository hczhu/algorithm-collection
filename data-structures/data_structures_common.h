...
const uint32_t ADDRESS_TYPE_MASK =
    ((((uint32_t)1) << ADDRESS_TYPE_BIT_WIDTH) - 1)
    << (32 - ADDRESS_TYPE_BIT_WIDTH);
...
struct signature64_t {
  ...
  inline uint64_t combine() const {
    return (((uint64_t)sig2) << 32) ^ ((uint64_t)sig1);
  };
...
//返回头四个字节组成的uint32_t
template <typename Usr_data_t> struct default_extract_uint32_key_t {
  uint32_t operator()(const Usr_data_t &usr_data) {
    return ((uint32_t *)(&usr_data))[0];
  };
};
//返回头四个字节组成的signature64_t
template <typename Usr_data_t> struct default_extract_signature64_key_t {
...
template <typename Usr_data_t, typename Hash_key_t> struct hashtable_trait_t {};
template <typename Usr_data_t> struct hashtable_trait_t<Usr_data_t, uint32_t> {
