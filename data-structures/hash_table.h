...
template<typename Usr_data_t,typename Hash_key_t,
		 typename Extract_key_func_t=typename hashtable_trait_t<Usr_data_t,Hash_key_t>::Extract_key_func_t,
		 typename Hash_func_t=typename hashtable_trait_t<Usr_data_t,Hash_key_t>::Hash_func_t >
class hashtable_t
{
public:
...
private:
	...
	void get_next(uint32_t& bucket_pos,Vaddress32& vptr)
	{
		...
		if(vptr!=NULL_VADDRESS32) return;
		bucket_pos++;
		while(bucket_pos<m_bucket_size&&NULL_VADDRESS32==(vptr=m_p_hash_bucket[bucket_pos])) {
			bucket_pos++;
		}
	};
public:
	inline uint32_t get_mem_type()
	{
		return m_mem_type;
	}
	struct Hash_node_t
	...
	class iterator_t 
	{
		...
		{
			return bucket_pos==other.bucket_pos&&vptr==other.vptr&&p_hash==other.p_hash;
		};
		bool operator!=(const iterator_t& other)
		{
			return bucket_pos!=other.bucket_pos||vptr!=other.vptr||p_hash!=other.p_hash;
	...
	{	
		...
		ret.bucket_pos=0;
		ret.p_hash=this;
		while(ret.bucket_pos<m_bucket_size&&(ret.vptr=m_p_hash_bucket[ret.bucket_pos])==NULL_VADDRESS32) {
			ret.bucket_pos++;
		}
		return ret;
	};
	...
	hashtable_t():m_p_hash_bucket(NULL),m_p_mem_pool(NULL),m_bucket_size(0),m_node_num(0),
					m_protect_dirty(true){};
			...
			bool protect_dirty=true,bool uniq=true) 
	{
		if(sizeof(Hash_node_t)!=sizeof(Vaddress32)+sizeof(Usr_data_t)) {
			ul_writelog(UL_LOG_WARNING,"There might be alignment problem in hashtable_t");
		}
		try
		{
		...
		{
			ul_writelog(UL_LOG_FATAL,"Allocating hash bucket of size %u failed",bucket_size);
			return -1;
		}
		m_bucket_size=bucket_size;
		m_p_mem_pool=&mem_pool;
		if(m_p_mem_pool->add_fixed_size_pool(sizeof(Hash_node_t),uniq)) {
			ul_writelog(UL_LOG_FATAL,"add_fixed_size_pool with size %u for hashtable_t failed",
						sizeof(Hash_node_t));
	...
	int clear()
	{
		...
		for(iterator_t itr=begin();itr!=end;itr++)
			m_p_mem_pool->free(combine_vaddr(m_mem_type,get_vaddr_offset((*itr)->vptr)),sizeof(Hash_node_t));
		return 0;
	}
	Vaddress32 seek_node_vaddr(const Hash_key_t& key)
	{
		for(Vaddress32 vptr=m_p_hash_bucket[m_hash_func(key,m_bucket_size)];NULL_VADDRESS32!=vptr;
				vptr=(convert(vptr))->next) {
	...
	Usr_data_t* seek_node(const Hash_key_t& key)
	{
		...
		if(NULL_VADDRESS32==addr) return NULL;
	...
	Vaddress32 add_node_vaddr(const Hash_key_t& key,const Usr_data_t& usr_data,bool overwrite)
	{
		...
		if(NULL_VADDRESS32==vptr) {
			ul_writelog(UL_LOG_FATAL,"Malloc memory of length %u failed from mem_pool",sizeof(Hash_node_t));
			return NULL_VADDRESS32;
		}
		ptr=convert(vptr);
	...
	//overwrite为真表示如果要加入的key已经存在，则替换为新的usr_data；为假则不替换
	Usr_data_t* add_node(const Hash_key_t& key,const Usr_data_t& usr_data,bool overwrite)
	{
	...
	Usr_data_t* remove_node(const Hash_key_t& key)
	{
		...
		if(NULL_VADDRESS32==ret) return NULL;
	...
	int get_detect_info(detect_info_t& detect_info)
	{
		...
		for(uint32_t pos=0;pos<m_bucket_size;pos++) {
			uint32_t length=0;
			for(Vaddress32 vptr=m_p_hash_bucket[pos];NULL_VADDRESS32!=vptr;vptr=convert(vptr)->next){
				length++;
			}
			longest=std::max(longest,length);
			node_num+=length;
			if(length)used_bucket++;
		}
		detect_info.node_num=node_num;
		detect_info.used_bucket=used_bucket;
		detect_info.longest_list_length=longest;
		detect_info.load_factor=1.0*detect_info.used_bucket/detect_info.bucket_size;
		detect_info.average_list_length=1.0*detect_info.node_num/detect_info.used_bucket;
		
		if(node_num!=m_node_num) {
			ul_writelog(UL_LOG_WARNING,"[m_node_num:%u] does not match [node_num:%u]",m_node_num,node_num);
		}
		return 0;
	};
	int get_detect_info(char buf[],uint32_t buf_size)
	{
		...
		snprintf(buf,buf_size,"node number : %u\nbucket size : %u\nused_bucket : %u\nlongest list length : %u\nload factor : %.3lf\naverage_list_length : %.3lf\n",detect_info.node_num,detect_info.bucket_size,detect_info.used_bucket,detect_info.longest_list_length,detect_info.load_factor,detect_info.average_list_length);
		return 0;
	};
	...
	int rehash(uint32_t new_bucket_size)
	{
		...
		ul_writelog(UL_LOG_WARNING,"hashtable_t is about to rehash from [%u] to [%u]",
		...
		for(uint32_t pos=0;pos<m_bucket_size;pos++) {
			...
			while(head!=NULL_VADDRESS32) {
				...
				head=ptr->next;
				uint32_t new_pos=m_hash_func(m_extract_key_func(ptr->usr_data),new_bucket_size);
		...
		//如果hash bucket变大，则先切换bucket，再改变bucket_size;否则可能在一写多读的情况下出现访问越界
...
{
	public:
	inline int operator()(const User_data_t& data)
	{
		return data.a;
	};
};
...
{
	public:
	inline uint32_t operator()(int a,uint32_t m)
	{
		return a%m;
	};
};
...
void hashtable_tester()
{
	...
	assert(res==0);
	User_data_t data;
	const char name[][14]={"aaa","bbbb","ccccc","dddddd","eeeeeee","fffffffffff"};
	char buf[1000];
	for(int i=0;i<N/2;i++)
	{
		data.a=i;
	...
	debug(buf);
	int cc=0;
	for(hashtable_t<User_data_t,uint32_t>::iterator_t itr=hashtable.begin();itr!=hashtable.end();itr++)
	{
		cc++;
		int key=(*itr)->a;
		assert((*itr)->b==key/2);
