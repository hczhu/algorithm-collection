...
template<typename Key,typename Value,typename Hashfunc=simple_hashfunc_t<Key> >
class hashtable_t
{
  ...
  int create(int size,const Key& invalid,bool init=false)
  {
    ...
    _node=0;
    _top=1;
    if(init)for(int i=0;i<=size;i++)table[i].key=_invalid_key;
    return 0;
  };
  ...
  char* monitor()
  {
    ...
    {
      if(table[i].key==_invalid_key)continue;
      fill++;
      Key key=table[i].key;
      uint pos=1+(hashf(key)%(_size-1));
      int step=1;
      while(table[pos].key!=key)step++,pos=table[pos].next;
      sum+=step;
      max_step=max(max_step,step);
    }
    snprintf(buffer,sizeof(buffer),"max access time: %d\naverage access time: %lf\nnode number: %d\nload factor %lf\n",max_step,1.0*sum/_node,_node,1.0*fill/_size);
    return buffer;
  };
  bool insert(const Key& key,Value value)
  {
    ...
    while(table[_top].key!=_invalid_key)_top++;
  ...
  void clear()
  {
    for(int i=0;i<=_size;i++)table[i].key=_invalid_key;
    _size=0;
  };
