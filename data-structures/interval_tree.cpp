...
template <int N, int L = (int)(ceil(log(1.0 * N) / log(2.0))),
          typename T_key = int, typename T_value = int>
...
public:
  ...
  // Return the sum of _value[i] such that _key[i]<=key_upper and
  ...
  T_value query(int start_index, int end_index, const T_key &key_upper) const {
    ...
    if (start_index)
      res -= query(start_index - 1, key_upper);
    return res;
  };
...
int interval_tree_t<L, N, T_key, T_value>::bsearch_key(
    const T_key &key_upper) const {
  const int *sindex = sorted_index[0];
  if (_key[sindex[0]] > key_upper)
    return -1;
  int low = 0, high = n - 1;
  while (low < high) {
    int mid = 1 + ((low + high) >> 1);
    if (_key[sindex[mid]] <= key_upper)
      low = mid;
    else
      high = mid - 1;
  }
...
template <int L, int N, typename T_key, typename T_value>
T_value
interval_tree_t<L, N, T_key, T_value>::query(int end_index,
                                             const T_key &key_upper) const {
  ...
  for (; pos >= 0;) {
    ...
    if (end_index <= mid) {
      ...
      if (left_link != -1)
        res += prefix_sum[level + 1][low + left_link];
      low = mid + 1;
      pos = right_link;
    }
    level++;
...
template <int L, int N, typename T_key, typename T_value>
void interval_tree_t<L, N, T_key, T_value>::build_tree_helper(int level,
                                                              int low,
                                                              int high) {
  assert(level < L);
  ...
  for (int ltop = 0, rtop = 0, top = 0; ltop < lsize || rtop < rsize;) {
    if (rtop == rsize)
      sindex[top++] = lindex[ltop++];
    else if (ltop == lsize)
      sindex[top++] = rindex[rtop++];
    else if (_key[lindex[ltop]] < _key[rindex[rtop]])
      sindex[top++] = lindex[ltop++];
    else
      sindex[top++] = rindex[rtop++];
  }
...
template <int L, int N, typename T_key, typename T_value>
void interval_tree_t<L, N, T_key, T_value>::build_tree(int size,
                                                       const T_key key[],
                                                       const T_value value[]) {
  n = size;
  _key = key;
