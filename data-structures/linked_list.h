...
#define SET_POINTER(vptr,prev_or_next,new_value) {double_pointer_t* tmp_pointer=get_pointer_func((vptr));CHECK_NULL_POINTER(tmp_pointer,-1);tmp_pointer->prev_or_next=(new_value);}
//改变第一个参数本身
#define GET_POINTER(dest,vptr,prev_or_next) {double_pointer_t* tmp_pointer=get_pointer_func(vptr);\
	CHECK_NULL_POINTER(tmp_pointer,-1);(dest)=tmp_pointer->prev_or_next;}
...
public:
	...
	int add_node(Vaddress32 vptr_list,Vaddress32 vptr_node)
	{
		...
		//空链表
			...
			dpointer->prev=combine_vaddr(INVALID_VADDR_TYPE,get_vaddr_offset(vptr_list));
		}
	...
	int remove_node(Vaddress32 vptr_node,Vaddress32& v_list_head_out)
	{
		...
		if(dpointer->prev==NULL_VADDRESS32) {
			ul_writelog(UL_LOG_WARNING,"The node [%u] is not in any doubly_linked_list_unsorted_t",
								vptr_node);
			return 0;
		}
		//非链表首节点
...
class doubly_linked_list_sorted_t : public doubly_linked_list_unsorted_t<Get_list_head_t,Get_pointer_t>
{
	Node_comparator_t node_comparator;			
	typedef doubly_linked_list_unsorted_t<Get_list_head_t,Get_pointer_t> Base_t;
	using doubly_linked_list_unsorted_t<Get_list_head_t,Get_pointer_t>::get_list_head_func;
	using doubly_linked_list_unsorted_t<Get_list_head_t,Get_pointer_t>::get_pointer_func;
	int insert_node_after(Vaddress32 v_pre_node,Vaddress32 v_add_node)
	{
		...
		for(;;)
		{
			...
			if(NULL_VADDRESS32==v_next_node||node_comparator(v_add_node,v_next_node)<=0) {
				break;		
			}
...
public:
	...
	int remove_sorted_node(Vaddress32 v_remove_node,Vaddress32& v_list_head_out)
	{
	...
	int add_sorted_node(const Vaddress32 vptr_list,const Vaddress32 vptr_node)
	{
		...
		if(NULL_VADDRESS32==*p_list_head) {
			...
			p_add_node_pointer->prev=combine_vaddr(INVALID_VADDR_TYPE,get_vaddr_offset(vptr_list));
	...
	int change_sorted_node(Vaddress32 v_old_node,Vaddress32 v_new_node)
	{
		...
		if(INVALID_VADDR_TYPE==get_vaddr_type(v_pre_node)) {
			...
			if(remove_sorted_node(v_old_node)||add_sorted_node(v_list_head,v_new_node)) {
				return -1;
			}
			return 0;
		}
		...
		//先向前移动
		while(get_vaddr_type(v_pre_node)!=INVALID_VADDR_TYPE&&node_comparator(v_pre_node,v_new_node)>0) {
...
void doubly_linked_list_unsorted_tester()
{
	...
	doubly_linked_list_unsorted_t<get_list_head_t,get_pointer_t> doubly_linked_list_unsorted;
	...
	res=mem_pool.create();
	assert(0==res);
	res=hash_node.create(std::max(10,N/10),mem_pool);assert(0==res);
	res=hash_list.create(std::max(10,N/10),mem_pool);assert(0==res);
	std::map<int,int> mem;
	node_t node;
	list_t list;
	int node_num=0;
	for(int i=0;i<N;i++)
	{
		...
		if(NULL!=hash_node.seek_node(node.sig64))continue;
		...
		mem[node.sig64.sig1]++;
	//	std::cerr<<"Add node:"<<node.sig64.sig1<<" "<<node.sig64.sig2<<std::endl;
	}
	debug(node_num);
	for(hashtable_t<node_t,signature64_t>::iterator_t itr=hash_node.begin();itr!=hash_node.end();itr++)
	{
		if(1&rand())continue;
	//	std::cerr<<"Remove node from list:"<<(*itr).sig64.sig1<<" "<<(*itr).sig64.sig2<<std::endl;
...
//		assert(res==-2);
	...
	for(hashtable_t<list_t,uint32_t>::iterator_t itr=hash_list.begin();itr!=hash_list.end();itr++)
	{
		uint32_t key=(*itr)->key;
		Vaddress32 list=(*itr)->list;
		int cnt=0;
		if(doubly_linked_list_unsorted.is_empty(list))continue;
	...
	//	std::cerr<<"Remove node from list:"<<(*itr).sig64.sig1<<" "<<(*itr).sig64.sig2<<std::endl;
		Vaddress32 vptr=hash_node.seek_node_vaddr((*itr)->sig64);
		if((*itr)->pointer.prev==NULL_VADDRESS32)continue;
		assert(vptr!=NULL_VADDRESS32);
		int res=doubly_linked_list_unsorted.remove_node(vptr);
		assert(res==0);
		(*itr)->pointer.prev=NULL_VADDRESS32;
...
struct node_comparator_t
{
	int operator()(Vaddress32 v_node1,Vaddress32 v_node2)
	{
		...
		if(p_node1->value==p_node2->value) return 0;
...
void doubly_linked_list_sorted_tester()
{
	...
	doubly_linked_list_sorted_t<get_list_head_t,get_pointer_t,node_comparator_t> doubly_linked_list_sorted;
