...
const uint32_t MASK1 = ((uint32_t)((1 << WIDTH1) - 1))
                       << (WIDTH2 + WIDTH3); //析取三个值的掩码
...
private:
  ...
  inline int new_block() {
    if (alloc_num < BLOCK_NUM) {
      ...
      if (0 == (item_size & 7)) {
        new_block = (char *)(new (
            std::nothrow) long long int[block_size * (item_size >> 3)]);
      }
      // 4字节对齐
      else if (0 == (item_size & 3)) {
        new_block =
            (char *)(new (std::nothrow) int[block_size * (item_size >> 2)]);
      }
      // 2字节对齐
      else if (0 == (item_size & 1)) {
        new_block =
            (char *)(new (std::nothrow) short[block_size * (item_size >> 1)]);
      } else {
        new_block = (new (std::nothrow) char[block_size * item_size]);
      }
      block_array[alloc_num++] =
          new_block; // new(std::nothrow) char[block_size * item_size];
...
public:
  ...
  inline uint32_t get_mem_consume() { return mem_consume; }
  inline uint32_t get_free_mem() { return free_mem + avail_num * item_size; }
  ...
  int create(const uint32_t block_size_t = 1 << WIDTH3,
             const uint32_t item_size_t = MIN_ITEM_SIZE) {
    mem_consume = 0;
    free_mem = 0;
  ...
  inline void back(const uint32_t p) {
    ...
    if (src != NULL) {
      free_mem += item_size;
      ((uint32_t *)real_address(p))[0] = free_list, free_list = p;
    } else {
...
public:
  ...
  int add_fixed_size_pool(uint32_t size, bool uniq = true,
                          uint32_t block_size = 1 << WIDTH3) {
    ...
    if (fixed_size_mem_pool_num >= MAX_TYPE_NUM) {
      ul_writelog(UL_LOG_FATAL,
                  "fixed_size_mem_pool_num reaches the upper bound [%d]",
                  MAX_TYPE_NUM);
      return -1;
    }
    ...
    if (mp[i]->create(block_size, size)) {
      delete mp[i];
      ul_writelog(UL_LOG_FATAL,
                  "fail to create fixed_size_mem_pool for bloack size=%u",
                  fixed_size_array[i]);
      return -1;
    }
    fixed_size_array[fixed_size_mem_pool_num] = size;
    sorted_fixed_size_array_index[fixed_size_mem_pool_num] =
    ...
    for (int j = fixed_size_mem_pool_num; j > 0; j--) {
      if (fixed_size_array[sorted_fixed_size_array_index[j]] <
        ...
        std::swap(sorted_fixed_size_array_index[j],
                  sorted_fixed_size_array_index[j - 1]);
      } else {
        break;
      }
    }
  ...
  inline uint32_t get_corresponding_type(uint32_t size) {
    ...
    if (size >
        fixed_size_array[sorted_fixed_size_array_index[fixed_size_mem_pool_num -
                                                       1]]) {
      ul_writelog(UL_LOG_FATAL,
                  "[size:%u] is too large in mem_pool_t::get_vaddr_type", size);
      return MAX_TYPE_NUM;
    }
  ...
  Vaddress32 malloc_fixed_size(uint32_t type) {
    if (type >= fixed_size_mem_pool_num) {
      ul_writelog(UL_LOG_FATAL,
                  "[type:%u] exceeds the current [fixed_size_mem_pool_num:%u]",
  ...
  inline Vaddress32 malloc(uint32_t size) {
    ...
    if (size >
        fixed_size_array[sorted_fixed_size_array_index[fixed_size_mem_pool_num -
                                                       1]]) {
      ul_writelog(UL_LOG_FATAL,
                  "Allocating [size:%u] is too large in mem_pool_t::malloc",
                  size);
      return NULL_VADDRESS32;
    }
  ...
  // tot_allocated:
  // 已分配出的内存字节数，不包括被保护的脏数据，等于tot_used加上浪费的字节数目
  ...
  void get_detect_info(char *ret, int max_len) {
    if (ret != NULL) {
      ...
      snprintf(ret, max_len,
               "total used: %ld;total allocated : %ld;total dirty size: %ld;\
total mem consume : %ld;total free space : %ld",
  ...
  int create(const uint32_t *lens, const uint32_t n,
    ...
    fixed_size_mem_pool_num = (uint32_t)(
        std::unique(fixed_size_array, fixed_size_array + n) - fixed_size_array);
    if (fixed_size_mem_pool_num > MAX_TYPE_NUM) {
      ul_writelog(UL_LOG_FATAL,
                  "[fixed_size_mem_pool_num:%d] goes beyond the upper bound \
        [MAX_TYPE_NUM:%d]",
                  fixed_size_mem_pool_num, MAX_TYPE_NUM);
      return -1;
    }
    ...
    m_p_dynamic_dirty_array =
        new (std::nothrow) im_vector_t<Vaddress32>(0xfffffff);
    if (NULL == m_p_dynamic_dirty_array) {
    ...
    if (m_p_dynamic_dirty_array->create(dynamic_dirty_array_size)) {
      ul_writelog(UL_LOG_FATAL,
                  "fail to create dynamic_dirty_array of initial size %u",
                  dynamic_dirty_array_size);
      return -1;
    }
  ...
  inline int free(Vaddress32 p, int used_length = -1, bool protect_dirty = true,
                  bool tail_writeable = false) {
    if (p == NULL_VADDRESS32) {
      return -1;
    }
    uint32_t type = 0;
    if ((type = get_vaddr_type(p)) >= fixed_size_mem_pool_num) {
      ul_writelog(UL_LOG_FATAL, "Free invalid Vaddress32 %u with type %u", p,
                  get_vaddr_type(p));
      return 0;
    }
    if (used_length == -1) {
    ...
    if (m_p_dynamic_dirty_array->push_back(p)) {
      ul_writelog(UL_LOG_FATAL, "Failed to push_back dirty Vaddress32 to "
                                "dynamic_dirty_array with dynamic_dirty_array "
                                "size=%u",
                  m_p_dynamic_dirty_array->size());
      return -1;
    }
    return 0;
  }
  ...
  void recycle() {
    ...
    for (uint32_t i = 0; i < vsize; i++) {
      mp[get_vaddr_type((*m_p_dynamic_dirty_array)[i])]->back(
          (*m_p_dynamic_dirty_array)[i]);
    }
...
void mem_pool_tester() {
  ...
  const int N = 5;
  const char name[N][24] = {"zhuhongcheng", "zhuhong", "chengzhuh",
                            "hongchengzhuhongcheng", "abcdefghi"};
  Vaddress32 vaddr[N];
  mem_pool_t pool;
  int res = pool.create();
  for (int i = 0; i < 10; i++) {
  ...
  for (int cc = 0; cc < 5; cc++) {
    ...
    //  assert(b==allocated);
    for (int i = 0; i < N; i++)
      pool.free(vaddr[i], strlen(name[i]) +
  ...
  for (int i = 0; i < 10000000; i++) {
    if (have.size() && 0 == (rand() % 3)) {
      ...
      Vaddress32 addr;
      if (1 & rand()) {
        addr = (rand() & 1) ? pool.malloc(sizeof(Node))
                            : pool.malloc_fixed_size(type);
  ...
  for (int i = 0; i < have.size(); i++) {
    if (NULL_VADDRESS32 != have[i]) {
      ...
      if (rand() & 1)
        res = pool.free(have[i], sizeof(Node)), have[i] = NULL_VADDRESS32;
      else
        res = pool.free(have[i]), have[i] = NULL_VADDRESS32;
      assert(res == 0);
    }
  }
