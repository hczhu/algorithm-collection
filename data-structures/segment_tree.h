...
//Node_t::update(Node_t& a,Node_t& b) is called when an interval overlaps with the updated interval.
//Node_t::equal(Node_t& a) is called when the interval which is being queried contained 
//the corresponding interval of a.
//Node_t::contained(Node_t& a) is called when the interval which is being queried is contained 
...
class segment_tree_t {
  ...
  void query_helper(int start,int end,tree_node_t& result,int low,int high,int idx) {
    while(low!=start||high!=end) {
      ...
      if(mid>=end) {
        ...
        if(start<=mid)query_helper(start,mid,result,low,mid,ll);
        low=mid+1;
 ...
 /* 
  ...
  void update_helper(int start,int end,int low,int high, int idx,tree_node_t& new_node) {
    ...
    int rr=ll+1;
    _tree[idx].update(_tree[ll],_tree[rr],low,high);
    if(start<=mid)update_helper(start,min(mid,end),low,mid,ll,new_node);
    if(end>mid)update_helper(max(start,mid+1),end,mid+1,high,rr,new_node);
    _tree[idx].update(_tree[ll],_tree[rr],low,high);
  };
};
template<typename T>
struct sum_node_t : tree_node_t {
    bool covered;
    T s,v,add;
    ...
    virtual void print(int idx) {
      cerr<<"node #"<<idx<<"=covered:"<<covered<<" s:"<<s<<" add:"<<add<<endl;
    }
  };
};
