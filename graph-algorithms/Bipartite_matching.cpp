...
#include <vector>
using namespace std;
template <typename T> struct trait_t {};
template <> struct trait_t<int> { const static int inf = 1000000000; };
template <> struct trait_t<long long int> {
  const static long long int inf = 1000000000000000000LL;
};
...
template <int N> class bipartite_match_t {
public:
  int rset[N];
  int rmatch[N];
  int ff;
