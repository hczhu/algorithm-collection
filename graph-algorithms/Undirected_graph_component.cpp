...
int low[N];
int flag[N];
void biconnectedComponents(int n, int m, int end1[], int end2[],
                           vector<vector<int>> &components) {
  components.clear();
  memset(edgeUsed, false, sizeof(bool) * m);
  for (int i = 0; i < m; i++)
    aa[i << 1] = end1[i], bb[i << 1] = end2[i], aa[(i << 1) ^ 1] = end2[i],
            bb[(i << 1) ^ 1] = end1[i];
  forwardStar(n, m * 2);
  ...
  for (int s = 0; s < n; s++) {
    ...
    while (top) {
      int v = stack[--top];
      int u;
      while (begin[v] < etop[v] &&
             (edgeUsed[edge[begin[v]] >> 1] || flag[u = bb[edge[begin[v]]]])) {
