...
// dis[k][v]: the shortest path from a feak source node s to node v consisting
// of exactly k edges. MMWC = min  { max { (dis[n][v]-dis[k][v])/ (n-k) | k } |
// v  } return the cycle represented as a serie of nodes. To-do: find such a
// cycle!!!!
...
double findMMWC(const vector<vector<pair<int, Len>>> &edge) {
  ...
  for (int v = 0; v < n; v++)
    if (dis[n][v] < inf / 2) {
      ...
      for (int k = 0; k < n; k++) {
        if (dis[k][v] < inf / 2 &&
            max_a * (n - k) < (dis[n][v] - dis[k][v]) * max_b) {
