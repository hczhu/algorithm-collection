...
void LCA_DFS(int v, const vector<vector<int>> &son,
             const vector<pair<int, int>> &want,
...
vector<int> offline_LCA(const vector<vector<int>> &son,
                        const vector<pair<int, int>> &want, int tree_root) {
  ...
  for (int i = 0; i < want.size(); i++) {
    ...
    if (want[i].first != want[i].second)
      rlist[want[i].second].push_back(i);
  }
