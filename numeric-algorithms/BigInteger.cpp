...
public:
  ...
  {
    if(head!=other.head)return head>other.head?1:-1;
    for(int i=head;i>=0;i--)
      if(dig[i]!=other.dig[i])return dig[i]>other.dig[i]?1:-1;
    return 0;
  };
  ...
  void set_value(const char* str,
        const char* sym="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
  {
    assert(strlen(sym)>=Base);
    //if(strlen(sym)<Base)Throw("Too small sym table in set_value");
    int width=0;
    for(uint p=base;p>1;p/=Base)width++;
    head=-1,sig=0;
    if(str[0]=='-')sig=1,str++;
  ...
  void abs_subtract(const BigInteger& b)
  {
    ...
    if(ca)
    {
      for(int i=b.head+1;i<=head;i++)
      {
        if(dig[i])
        {
          for(int j=b.head+1;j<i;j++)dig[j]=base-1;
          dig[i]--;
          ca=0;
          break;
        }
  ...
  {
    int ca=0;
    while(head<b.head)dig[++head]=0;
    for(int i=0;i<=b.head;i++)
    {
      dig[i]+=ca+b.dig[i];
      ca=0;
      if(dig[i]>=base)ca=1,dig[i]-=base;
    }
    if(ca)
    {
      for(int i=b.head+1;i<=head;i++)
      {
        ...
        {
          dig[i]++;
          for(int j=b.head+1;j<i;j++)dig[j]=0;
          ca=0;
          break;
        }
      }
  ...
  const BigInteger& operator+=(const BigInteger& b)
  {
    if(sig^b.sig)
    {
      if(abs_compare(b)>=0)abs_subtract(b);
      else
      {
        BigInteger tmp=b;
  ...
  {
    if(sig^other.sig)return sig==0?1:-1;
    return (sig==0?1:-1)*abs_compare(other);
  };
  void print(const char *tail="",FILE* file=stdout,
      const char* sym="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")const
  {
    assert(strlen(sym)>=Base);
    if(sig)fprintf(file,"-");
    char seg[64];
    int top=0;
    for(uint d=dig[head];d;d/=Base)seg[top++]=sym[d%Base];
    reverse(seg,seg+top);seg[top]=0;
    fprintf(file,"%s",seg);
    int width=0;
    for(uint p=base;p>1;p/=Base)width++;
    seg[width]=0;
