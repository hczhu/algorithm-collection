...
s.t.  x>=0 &&  A*x=b
*/
template <int N, int M> class LPSimplex {
  void printAll(double ma[][N], double vb[], double vc[], int m, int n,
                int basic[]) {
    puts("-------ma and vb---------------");
  ...
  void pivot(double ma[][N], double vb[], double vc[], int m, int n,
             int basic[], int leave, int enter) {
#ifdef OUTPUT
    printf("---pivot %d--->%d\n", leave, enter);
#endif
  ...
  bool simplex(double ma[][N], double vb[], double vc[], int m, int n,
               int basic[]) {
    ...
    if (basic[n + m] != -1) {
      ...
      for (enter = 0; enter < n + m; enter++)
        if (fabs(ma[ind][enter]) > EPS)
          break;
      if (enter < n + m)
        pivot(ma, vb, vc, m, n + m + 1, basic, n + m, enter);
    }
    delete[] vc;
    return true;
  }
  int LP(double ma[][N], double vb[], double vc[], int m, int n, int basic[]) {
    if (!initSolution(ma, vb, m, n, basic))
      return -1;
    // return 0;
    for (int i = n; i < n + m; i++)
      vc[i] = 0.0;
...
#endif
    if (!simplex(ma, vb, vc, m, n + m, basic))
      return 1;
    return 0;
  }
};
