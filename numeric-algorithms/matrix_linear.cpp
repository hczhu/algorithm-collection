...
template <typename T> Matrix<T> Matrix<T>::power(long long int s) const {
...
template <typename T> Matrix<T> Matrix<T>::power_sum(long long int s) const {
  assert(this->size == this->at(0).size());
  if (s == 1)
    return *this;
  if (s == 0)
...
int GuassElimination(vector<vector<double>> aa, vector<double> bb,
  ...
  for (int k = 0, p = 0; k < n && p < m; k++) {
    int best = p;
    for (int i = p; i < m; i++)
      if (fabs(aa[i][k]) > fabs(aa[best][k]))
        best = i;
    if (fabs(aa[best][k]) < eps)
      continue;
    for (int i = k; i < n; i++)
      swap(aa[best][i], aa[p][i]);
    swap(bb[best], bb[p]);
    major.push_back(k);
    bb[p] /= aa[p][k];
    for (int i = n - 1; i >= k; i--)
