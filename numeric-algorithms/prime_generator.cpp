...
bool isPrime(uint n) {
	...
	while((upper+1)*(upper+1)<=n)upper++;
	for(uint x=2;x<=upper;x++) {
		if(0==(n%x))return false;
	}
	return true;
}
...
uint* generate_prime(uint n) {
	...
	uint p=1,euler=1;
	for(int i=0;i<prime_seed_number;i++) p*=prime_seed[i],euler*=prime_seed[i]-1;
	debug(euler);
	debug(p);
		...
		for(int i=0;i<euler;i++,idx++) {
			...
			if(0==(Two((idx&31))&bit_mask[idx>>5])) {
				if((++pidx%step)==0) {
					...
					for(int j=d/2-1;j>=0;j--)swap(out_buf[out_top+j],out_buf[out_top+d-1-j]);
					out_top+=d;
					out_buf[out_top++]='\n';
