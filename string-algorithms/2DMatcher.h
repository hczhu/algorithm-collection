...
template <typename T> class twoD_matcher_t {
public:
  struct match_t {
    int r, c, idx;
    bool operator<(const match_t &b) const { return idx < b.idx; };
    ...
    for (int c = 0; c < text[0].length(); c++) {
      ...
      if (c + 1 >= m) {
        ...
        for (int r = 0; r < compact.size(); r++) {
          ...
          for (int mst = one_tree.isMatched(st) ? st
                                                : one_tree.getMatchlink(st);
               mst > 0; mst = one_tree.getMatchlink(mst)) {
    ...
    for (int r = 0; r < text.size(); r++) {
      for (int c = 0; c < text[r].length(); c++) {
        for (int i = 0; i < pat.size(); i++) {
          ...
          bool ok =
              (r + let.size() <= text.size()) && (c + m <= text[0].length());
...
#endif
    std::cerr << "match column size=" << m << " pattern " << res.size()
              << std::endl;
    return res;
  }
};
