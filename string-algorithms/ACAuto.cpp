...
template <typename T> class Convertor {
  static const int Hsize =
      (sizeof(T) >= 4 || (1 << 16) < (1 << (sizeof(T) * 8)))
          ? (1 << 16)
          : (1 << (8 * sizeof(T)));
  int hash[Hsize];
  std::map<T, int> mem;
...
public:
  ...
  int dump(FILE *fout) const {
    ...
    for (typename std::map<T, int>::iterator itr = mem.begin();
         itr != mem.end(); itr++) {
      if (1 != fwrite(&(itr->first), sizeof(itr->first), 1, fout) ||
          1 != fwrite(&(itr->second), sizeof(itr->second), 1, fout))
        return -1;
    }
    return 0;
  }
  int load(FILE *fin) {
    ...
    for (size_t i = 0; i < mem_size; i++) {
      T a;
      int b;
      if (1 != fread(&a, sizeof(a), 1, fin) ||
          1 != fread(&b, sizeof(b), 1, fin))
        return -1;
...
template <> class Convertor<unsigned short> {
...
public:
  ...
  int dump(FILE *fout) {
    ...
    for (std::map<unsigned short, int>::const_iterator itr = mem.begin();
         itr != mem.end(); itr++) {
      if (1 != fwrite(&(itr->first), sizeof(itr->first), 1, fout) ||
          1 != fwrite(&(itr->second), sizeof(itr->second), 1, fout))
        return -1;
...
template <typename Char, typename Info> class ACAuto {
public:
  ...
  void removeCell(int pos) {
    ...
    if (next < (int)m_array.size())
      m_array[next].check = -pre;
    m_array[pos].check = 0;
    //  printf("remove %d\n",pos);
    //  print();
  ...
  void enlarge(uint size) {
    ...
    for (; m_array[tail].check >= 0 &&
           -m_array[head].base < (int)m_array.size();) {
  ...
  inline int getCharIndex(Char ch) const {
    return m_convertor.getCharIndex(ch);
  };
  ...
  int load(FILE *fin) {
    ...
    if (m_size != fread(tmp_pointer, sizeof(m_pArray[0]), m_size, fin))
      return -1;
    m_inited = true;
    return 0;
  }
  ...
  bool construct_helper(std::vector<int> &vidx, int parent, int dep) {
...
#ifdef DEBUG_OUTPUT
    std::cerr << "constructing dep " << dep << " size=" << vidx.size()
              << std::endl;
    ...
    for (int cell = -m_array[1].base;; cell = -m_array[cell].base) {
      ...
      bool ok = false;
      if (base + m_convertor.cnt > (int)m_array.size())
        enlarge(base + m_convertor.cnt);
      ok = true;
    ...
    assert(base >= 0);
#ifdef DEBUG_OUTPUT
    std::cerr << "Find base " << base << " for node " << parent
              << " with children ";
    for (int i = 0; i < (int)have.size(); i++)
      std::cerr << have[i] << " ";
    std::cerr << std::endl;
#endif
    if ((int)m_edge.size() <= parent)
      m_edge.resize(parent + 1);
    ...
    for (int i = 0, p = 0; i < (int)have.size(); i++) {
      ...
      while (p < (int)vidx.size() && (int)m_vstr[vidx[p]][dep] == have[i]) {
        if ((int)m_vstr[vidx[p]].size() == dep + 1) {
          if (m_array[base + have[i]].match) {
            // OUTFOX_LOG((LP_WARNING,"Duplicated strings appeared %d
            // %d\n",vidx[p],vidx[p-1]));
          }
      ...
      if (!construct_helper(son, base + have[i], dep + 1))
        return false;
    }
    return true;
  }
  bool construct_link() {
    ...
    while (qq.size()) {
...
#endif
      for (int i = 0; i < (int)have.size(); i++) {
        if (have[i] + base >= (int)m_array.size() ||
            m_array[base + have[i]].check != parent) {
          // OUTFOX_RETURN((LP_ERROR,"Internal error in
          // construct_link\n"),false);
          assert(0);
        }
      }
      ...
      for (int i = 0; i < (int)have.size(); i++) {
        ...
        if (m_array[pre].match)
          m_array[son].match_link = pre;
        else
          m_array[son].match_link = m_array[pre].match_link;
#ifdef DEBUG_OUTPUT
        printf("node[%d].fail=%d\n", base + have[i],
               m_array[base + have[i]].fail);
#endif
      }
    }
    return true;
...
public:
  void print() {
    ...
    for (int i = 0; i < (int)m_array.size(); i++) {
      ...
      std::cerr << "cell #" << i << ": "
                << "dep=" << m_array[i].dep << " check=" << m_array[i].check
                << " base=" << m_array[i].base << " fail=" << m_array[i].fail
                << " match=" << m_array[i].match << " info=" << m_array[i].info
                << std::endl;
      if (pre != -1) {
    ...
    for (int i = 0; i < (int)m_array.size(); i++) {
      ...
      std::cerr << "cell #" << i << ": "
                << "dep=" << m_array[i].dep << " check=" << m_array[i].check
                << " base=" << m_array[i].base << " fail=" << m_array[i].fail
                << " match=" << m_array[i].match
                << " match_link=" << m_array[i].match_link
                << " info=" << m_array[i].info << std::endl;
    }
  }
  ACAuto(uint size = 2)
      : m_pArray(NULL), m_inited(false), m_last_cell(0),
        m_array(std::max((uint)2, size)) {
  ...
  bool insertString(const Char *ptr, int length, const Info &info) {
    ...
    for (int i = 0; i < length; i++)
      m_vstr.back()[i] = GetCharIndex(ptr[i]);
#ifdef DEBUG_OUTPUT
    for (int i = 0; i < length; i++)
      std::cerr << m_vstr.back()[i];
    std::cerr << std::endl;
#endif
  ...
  inline bool IsInitialized() const { return (NULL != m_pArray && m_inited); }
...
// std::cerr<<"Construct DA ended"<<std::endl;
    ...
    // OUTFOX_RETURN((LP_ERROR,"Construct ACAuto of %zu words failed due to
    // memoey alloc\n",m_vstr.size()),false);
    assert(0);
  } catch (...) {
    // OUTFOX_RETURN((LP_ERROR,"Construct ACAuto of %zu words
    // failed\n",m_vstr.size()),false);
    assert(0);
  }
  ...
  inline int getNext(int st, Char ch) const {
    ...
    while (st) {
      int pos = m_pArray[st].base + idx;
      if (m_pArray[pos].check == st)
        return pos;
      st = m_pArray[st].fail;
    }
    if (st)
      return st;
    return (m_pArray[m_pArray[0].base + idx].check == 0)
               ? m_pArray[0].base + idx
               : 0;
  }
  ...
  bool isMatched(int st) const { return m_pArray[st].match; };
  size_t cell_size() { return m_array.size(); };
  size_t used_cell() {
    size_t used = 0;
    for (int i = 0; i < (int)m_array.size(); i++)
      if (m_array[i].check >= 0)
  ...
  size_t used_mem() { return m_array.capacity() * sizeof(AC_node); };
};
...
int main() {
  ...
  for (int i = 0; i < 1000000; i++) {
    ...
    if (mem.count(vector<short>(buf, buf + len)))
      continue;
  ...
  for (int i = 0; i < 1000000; i++) {
    ...
    for (int s = st; s; s = tree.getFail(s)) {
      if (tree.isMatched(s)) {
        ...
        for (int j = 0; j < len; j++)
          assert(seq[i - j] == all[pos][len - 1 - j]);
        assert(len < pre);
        pre = len;
        match.push_back(s);
      }
    ...
    for (int s = tree.isMatched(st) ? st : tree.getMatchlink(st); s;
         s = tree.getMatchlink(s)) {
      assert(match.size());
      assert(s == match.back());
      match.pop_back();
    }
    ...
    for (int len = 1; len <= i + 1 && len <= wlen; len++) {
      vector<short> pat =
          vector<short>(seq.begin() + i - len + 1, seq.begin() + i + 1);
      if (mem.count(pat))
        cnt--;
    }
    if (cnt) {
