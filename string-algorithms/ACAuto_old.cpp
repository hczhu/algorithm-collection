...
template <typename Char, typename Info> class ACAuto {
  ...
  int GetCharIndex(Char ch) {
    if (ch >= 0 && ch < sizeof(m_convertor.hash)) {
      if (-1 == m_convertor.hash[ch])
        m_convertor.hash[ch] = m_convertor.cnt++;
      return m_convertor.hash[ch];
    } else {
      if (m_convertor.mem.count(ch) == 0)
        m_convertor.mem[ch] = m_convertor.cnt++;
  ...
  bool construct_helper(std::vector<int> &vidx, int parent, int dep) {
    ...
    while (m_last_cell < m_array.size() && m_array[m_last_cell].check != -1)
      m_last_cell++;
    // m_last_cell=
    bool ok = false;
    int base;
    for (base = std::max(0, m_last_cell - have.back());; base++) {
      if (base + have.back() >= m_array.size())
        m_array.resize(base + have.back() + 1);
      ok = true;
    ...
    std::cerr << std::endl;
#endif
    if (base + m_convertor.cnt > m_array.size())
      m_array.resize(base + m_convertor.cnt);
    if (m_edge.size() <= parent)
    ...
    while (qq.size()) {
      ...
      for (int i = 0; i < have.size(); i++) {
        ...
        m_array[base + have[i]].fail =
            (m_array[next].check == fail) ? next : fail;
...
public:
  ...
  bool construct() try {
    ...
    OUTFOX_RETURN((LP_ERROR,
                   "Construct ACAuto of %u words failed due to memoey alloc\n",
                   m_vstr.size()),
                  false);
  } catch (...) {
    OUTFOX_RETURN(
        (LP_ERROR, "Construct ACAuto of %u words failed\n", m_vstr.size()),
        false);
  }
  int get_fail(int st) {
  ...
  bool isMatched(int st) { return m_pArray[st].match; };
};
int main() {
  srand(23123);
