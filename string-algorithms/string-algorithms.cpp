...
void extended_kmp(int pattern[], int n, int extend[]) {
  ...
  for (int i = 1; i < n; i++) {
    int p = 0;
    if (reach >= i)
      p = min(reach + 1 - i, extend[i - idx]);
    while (p + i < n && pattern[p] == pattern[p + i])
      p++;
    extend[i] = p;
    if (p + i - 1 > reach)
      reach = p + i - 1, idx = i;
...
void longest_common_prefix(int pattern[], int n, int text[], int m,
                           int extend[] = NULL) {
  bool allo = false;
  if (extend == NULL) {
    allo = true;
  ...
  for (int i = 0; i < m; i++) {
    int p = i;
    if (reach >= i)
      p = min(reach + 1, extend[i - idx] + i);
    while (p - i < n && pattern[p - i] == text[p])
      p++;
    text[i] = p - i;
    if (reach < p - 1)
      reach = p - 1, idx = i;
...
template <int ALPHA, int M> class StringsAutomaton {
  int dep[M];
...
public:
  ...
  inline int getNext(int r, char ch) { return next[r][hash[ch]]; };
...
void palindrom_extend(int str[], int n, int extend[]) {
  ...
  for (int i = 1; i < n; i++) {
    int len = 0;
    if (reach > i)
      len = min(reach - i, extend[2 * idx - i]);
...
template <typename Char> class AC_automaton_t {
public:
  struct edge_t {
    Char ch;
    int idx;
  ...
  struct node_t {
    ...
    node_t(int a, int b = 0, int c = 0) : dep(a), count(b), sum(c), failed(0) {
      edge.reserve(10);
    };
    void sort() { std::sort(edge.begin(), edge.end()); };
    void print() const {
      cerr << "dep: " << dep << " cout: " << count << " sum: " << sum
           << " failed: " << failed << " edges:";
      for (int i = 0; i < edge.size(); i++)
        cerr << " (" << edge[i].ch << "," << edge[i].idx << ")";
      cerr << endl;
    };
  };
  vector<node_t> nodes;
...
public:
  ...
  int find_next_sorted(const node_t &node, Char ch) const {
    edge_t out(ch, -1);
    int pos = (int)(lower_bound(node.edge.begin(), node.edge.end(), out) -
                    node.edge.begin());
    if (pos < node.edge.size() && node.edge[pos].ch == ch)
      return node.edge[pos].idx;
    return -1;
  };
  int find_next(const node_t &node, Char ch) const {
    for (int i = 0; i < node.edge.size(); i++)
      if (ch == node.edge[i].ch)
        return node.edge[i].idx;
    return -1;
  }
  ...
  int get_next_sorted(int st, Char ch) const {
    return find_next_sorted(nodes[st], ch);
  };
