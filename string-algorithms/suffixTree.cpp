...
int suffixRescan(int prefix, int prefixPos, int headPos, char *str, int &top) {
  ...
  while (prefixPos < headPos) {
    ...
    if (son >= 0) {
      for (int i = 0;
           i <= snode[son].end - snode[son].begin && i + prefixPos < headPos;
           i++)
        assert(str[i + snode[son].begin] == str[prefixPos + i]);
    } else {
      son = -son - 2;
      for (int i = 0;
           i <= enode[son].end - enode[son].begin && i + prefixPos < headPos;
           i++)
...
void suffixScan(int &head, int &headPos, char *str, int &top) {
  for (;;) {
    ...
    if (son >= 0) {
      ...
      for (len = snode[son].begin;
           len <= snode[son].end && str[len] == str[headPos]; len++, headPos++)
        ;
      assert(len > snode[son].begin);
      if (len <= snode[son].end) {
        addInternalNode(top, head, len - snode[son].begin,
                        str[snode[son].begin], str);
        head = top++;
        break;
      }
    } else {
      ...
      for (len = enode[son].begin;
           len <= enode[son].end && str[len] == str[headPos]; len++, headPos++)
        ;
      assert(len > enode[son].begin);
      if (len <= enode[son].end) {
        addInternalNode(top, head, len - enode[son].begin,
                        str[enode[son].begin], str);
        head = top++;
        break;
      }
      assert(0);
...
void printSnode(int ind) {
  puts("--------------------------");
  printf("snode%d parent%d %d->%d\n", ind, snode[ind].parent, snode[ind].begin,
         snode[ind].end);
  ...
  for (int i = 0; i < M; i++)
    if (snode[ind].next[i] != -1) {
      ...
      if (snode[ind].next[i] >= 0)
        ...
        printf("end %d %d %d\n", enode[nd].suffixIndex, enode[nd].begin,
               enode[nd].end);
      }
    }
  assert(ind == 0 || cc != 1);
  assert(ind == 0 ||
         snode[snode[ind].parent].next[str[snode[ind].begin]] == ind);
}
int constructSuffixTree(char *str, const int n) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (snode[head].suffixLink == -1) {
      ...
      if (prefixPos < i)
        prefixPos = i;
      snode[head].suffixLink =
          suffixRescan(prefix, prefixPos, headPos, str, top);
    }
