...
template <typename T> struct circular_queue_t {
  enum item_status_t {
    Null,    // No item here
...
public:
  circular_queue_t()
      : array(NULL), item_status(NULL), cap(0), head(0), tail(0){};
  ~circular_queue_t() {
    if (NULL != array)
      delete[] array;
    if (NULL != item_status)
...
template <typename T> int circular_queue_t<T>::create(int n) {
  if (n <= 1) {
  ...
  try {
    ...
    ul_writelog(UL_LOG_FATAL,
                "Allocating memory failed in circular_queue_t::create failed");
    if (NULL != array)
      delete[] array;
    return -1;
  }
...
private:
  ...
  int create_threads();
  inline int stop();
  int run0(); // num_processor processor threads, 1 thread for generator, 1
              // thread for consumer
  int run1(); // num_processor processor threads, 1 threads for both generator
              // and consumer
  int run2(); // num_processor processor threads, 1 threads for generator,
              // consumer and an extra processor
public:
  ...
  parallel_framework_t()
      : _processor_tid(NULL), _generator(NULL), _consumer(NULL),
        _processor(NULL), status(Null), num_processor(0), queue_size(0),
        generator_item_count(0), consumer_item_count(0),
        processor_item_count(NULL), processor_arg(NULL), mode(0),
        processor_return(NULL){};
  ~parallel_framework_t() {
...
template <typename T>
int parallel_framework_t<T>::create(int _num_processor, int _buf_size,
                                    int _mode) {
  if (_num_processor < 0 || _buf_size <= 0 || _mode < 0 || _mode > 2) {
    ul_writelog(
        UL_LOG_FATAL,
        "Incorrect arguments [num_processor:%d] [buf_size:%d] [mode:%d]",
        _num_processor, _buf_size, _mode);
    return -1;
  }
  if (_mode == 2)
    _num_processor--;
  if (_num_processor == 0 && _mode != 2) {
    ul_writelog(UL_LOG_FATAL,
                "Incorrect argument combination: [num_processor:%d] [mode:%d]",
                _num_processor, _mode);
    return -1;
  }
  if (NULL != _processor_tid)
    delete[] _processor_tid;
  ...
  try {
    ...
    processor_return =
        new int[(_num_processor + (_mode == 2 ? 1 : 0)) * _buf_size];
  } catch (std::bad_alloc) {
    if (NULL != _processor_tid)
      delete[] _processor_tid;
    if (NULL != processor_item_count)
    ...
    if (NULL != processor_return)
      delete[] processor_return;
    ul_writelog(UL_LOG_FATAL,
                "allocating memory failed in parallel_framework_t::create");
    return -1;
  }
  ...
  ul_writelog(UL_LOG_NOTICE, "parallel_framework_t::create succeeded with "
                             "[num_processor:%d] [buf_size:%d] [mode:%d] "
                             "[queue_size:%d]",
              _num_processor, _buf_size, _mode, queue_size);
  return 0;
}
// processor return: 0:succeed,-1:error, other:fatal error and the thread must
// exit,
template <typename T> void *thread_processor(void *arg) {
  ...
  typename circular_queue_t<T>::item_status_t *status =
      p_frame->queue.item_status;
  ...
  for (bool go = true; go;) {
    if (p_frame->status != parallel_framework_t<T>::Running)
      go = false;
    for (; circular_queue_t<T>::Waiting == status[pos];
         pos = (pos + inc) % queue_size) {
      p_frame->process_one_item(pos, pid);
    }
  }
  ul_writelog(UL_LOG_NOTICE, "processor #%d clock time: %lf, wall time: %lf",
              pid, get_clock_time() - clock_time, get_wall_time() - wall_time);
  return (void *)0;
}
...
template <typename T> void *thread_consumer(void *arg) {
  ...
  for (; p_frame->status == parallel_framework_t<T>::Running ||
         queue.has_item();) {
    while (queue.has_item() && p_frame->consume_one_item())
      ;
  }
  ul_writelog(UL_LOG_NOTICE, "consumer clock time: %lf, wall time: %lf",
              get_clock_time() - clock_time, get_wall_time() - wall_time);
  return (void *)0;
}
template <typename T> int parallel_framework_t<T>::create_threads() {
  status = Running;
  ...
  if (mode == 0) {
    if (0 > pthread_create(&_consumer_tid, NULL, thread_consumer<T>,
                           (void *)this)) {
    ...
    ul_writelog(UL_LOG_NOTICE, "No consumer thread is created for mode %d",
                mode);
  }
  return 0;
}
template <typename T> inline int parallel_framework_t<T>::generate_one_item() {
...
template <typename T> inline int parallel_framework_t<T>::consume_one_item() {
...
template <typename T>
int parallel_framework_t<T>::run(functionT generator, functionT processor,
                                 functionC consumer) {
  ...
  if (status != Ready) {
    ul_writelog(UL_LOG_FATAL, "Call parallel_framework_t::create before "
...
template <typename T> int parallel_framework_t<T>::run0() {
...
template <typename T> int parallel_framework_t<T>::run1() {
  int gen_res;
...
template <typename T> int parallel_framework_t<T>::run2() {
  ...
  ul_writelog(UL_LOG_NOTICE, "generator clock time: %lf, wall time: %lf",
              get_clock_time() - clock_time, get_wall_time() - wall_time);
  return 0;
}

template <typename T> inline int parallel_framework_t<T>::stop() {
  status = Ready;
