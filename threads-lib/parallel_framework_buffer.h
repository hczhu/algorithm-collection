...
public:
  ...
  parallel_framework_buffer_t()
      : p_buffer(NULL), buffer_status(NULL), sem_ready_buffer(NULL),
        sem_waiting_buffer(NULL), sem_null_buffer(NULL), _processor_tid(NULL),
        _generator(NULL), _consumer(NULL), _processor(NULL), status(frame_Null),
        num_processor(0), generator_item_count(0), consumer_item_count(0),
        processor_item_count(NULL), processor_arg(NULL){};
...
int parallel_framework_buffer_t<T>::create(int _num_processor, int _buf_size) {
  ...
  if (status != frame_Null) {
    ul_writelog(UL_LOG_FATAL, "parallel_framework_buffer_t::create has already "
  ...
  try {
    ...
    ul_writelog(
        UL_LOG_FATAL,
        "allocating memory failed in parallel_framework_buffer_t::create");
    return -1;
  }
  ...
  ul_writelog(UL_LOG_NOTICE, "parallel_framework_buffer_t::create succeeded "
...
template <typename T> void *thread_processor(void *arg) {
  ...
  for (;;) {
    ...
    if (p_frame->buffer_status[pid] !=
        parallel_framework_buffer_t<T>::buffer_Waiting) {
      break;
    }
  ...
  sem_post(p_frame->sem_ready_buffer + pid);
#ifdef PARALLEL_PRINT_TIME
  ul_writelog(UL_LOG_NOTICE, "processor #%d wait time=%lf post time=%lf", pid,
              wait_time, post_time);
#endif
...
template <typename T> void *thread_consumer(void *arg) {
  ...
  parallel_framework_buffer_t<T> *p_frame =
      (parallel_framework_buffer_t<T> *)arg;
  ...
  for (;;) {
    ...
    if (p_frame->buffer_status[buffer_pos] !=
        parallel_framework_buffer_t<T>::buffer_Ready) {
      break;
    }
...
#ifdef PARALLEL_PRINT_TIME
  ul_writelog(UL_LOG_NOTICE, "consumer wait time=%lf post time=%lf", wait_time,
              post_time);
#endif

  return (void *)0;
}
template <typename T> int parallel_framework_buffer_t<T>::create_threads() {
...
template <typename T>
int parallel_framework_buffer_t<T>::run(functionT generator,
                                        functionT processor,
                                        functionC consumer) {
  ...
  if (status != frame_Ready) {
    ul_writelog(UL_LOG_FATAL, "Call parallel_framework_buffer_t::create before "
...
template <typename T> int parallel_framework_buffer_t<T>::run() {
#ifdef PARALLEL_PRINT_TIME
  double pre_time, wait_time = 0, post_time = 0;
#endif
...
#ifdef PARALLEL_PRINT_TIME
  ul_writelog(UL_LOG_NOTICE, "generator wait time=%lf post time=%lf", wait_time,
              post_time);
#endif
