...
template<typename T>
struct parallel_framework_lock_t
{
  ...
  int mutex_dequeue()
  {
    ...
    if(head==tail) {
      if(status==frame_Running) {
        ul_writelog(UL_LOG_FATAL,"Empty queue in mutex_dequeue while framework is running");
      }
      ret=-1;
    }
    else {
...
int parallel_framework_lock_t<T>::create(int _num_processor,int _buf_size)
{
  ...
  if(status!=frame_Null) {
    ul_writelog(UL_LOG_FATAL,"parallel_framework_lock_t::create has already been called successfully");
    return -1;
  }
  try
  {
  ...
  catch(std::bad_alloc)
  {
    ...
    ul_writelog(UL_LOG_FATAL,"allocating memory failed in parallel_framework_lock_t::create");
    return -1;
  }
  ...
  ul_writelog(UL_LOG_NOTICE,"parallel_framework_lock_t::create succeeded with [num_processor:%d] [buf_size:%d] [queue_size:%d]",_num_processor,_buf_size,queue_size);
  return 0;
}
template<typename T>
void* thread_processor(void* arg)
{
  parallel_framework_lock_t<T>* p_frame=((processor_argument_t<T>*)arg)->p_frame;
  assert(NULL!=p_frame);
...
void* thread_consumer(void* arg)
{
  ...
  for(;;) {
    ...
    else if(p_frame->item_status[pos]==parallel_framework_lock_t<T>::item_Waiting) {
      ul_writelog(UL_LOG_FATAL,"A Ready item is expected by the consumer, but a Waiting item is encountered.");
      break;  
    }
...
template<typename T>
int parallel_framework_lock_t<T>::create_threads()
{
...
inline int parallel_framework_lock_t<T>::consume_one_item(int pos)
{
  ...
  if(st) {
    ul_writelog(UL_LOG_WARNING,"consumer failed to consume one item [return:%d].The item is skipped.",st);
  }
  else {
    consumer_item_count++;
  }
...
template<typename T>
int parallel_framework_lock_t<T>::run(functionT generator,functionT processor,functionC consumer)
{
  ...
  if(status!=frame_Ready) {
    ul_writelog(UL_LOG_FATAL,
        "Call parallel_framework_lock_t::create before calling parallel_framework_t::run");
    return -1;
  }
...
template<typename T>
int parallel_framework_lock_t<T>::run()
{
