...
public:
  ...
  parallel_framework_semaphore_t()
      : item_status(NULL), array(NULL), head(0), tail(0), sem_ready_cell(NULL),
        sem_waiting_cell(NULL), _processor_tid(NULL), _generator(NULL),
        _consumer(NULL), _processor(NULL), status(frame_Null), num_processor(0),
        queue_size(0), generator_item_count(0), consumer_item_count(0),
...
template <typename T>
int parallel_framework_semaphore_t<T>::create(int _num_processor,
                                              int _buf_size) {
  ...
  if (status != frame_Null) {
    ul_writelog(UL_LOG_FATAL, "parallel_framework_semaphore_t::create has "
  ...
  try {
    ...
    ul_writelog(
        UL_LOG_FATAL,
        "allocating memory failed in parallel_framework_semaphore_t::create");
    return -1;
  }
  ...
  ul_writelog(UL_LOG_NOTICE, "parallel_framework_semaphore_t::create succeeded "
                             "with [num_processor:%d] [buf_size:%d] "
                             "[queue_size:%d]",
...
template <typename T> void *thread_processor(void *arg) {
  ...
  for (;;) {
    ...
    assert(p_frame->item_status[pos] ==
           parallel_framework_semaphore_t<T>::item_Waiting);
...
#ifdef PRINT_TIME
  cpu_time += get_wall_time() - pre_cpu;
  ul_writelog(UL_LOG_NOTICE, "processor #%d CPU time: %lf, wall time: %lf", pid,
              cpu_time, get_wall_time() - wall_time);
#endif
  return (void *)0;
}
template <typename T> void *thread_processor_mode2(void *arg) {
  const double wall_time = get_wall_time();
  parallel_framework_semaphore_t<T> *p_frame =
      ((processor_argument_t<T> *)arg)->p_frame;
  assert(NULL != p_frame);
  ...
  //多post一次，用于结束时唤醒consumer
  sem_post(p_frame->sem_ready_cell + pid);
  ul_writelog(UL_LOG_NOTICE, "processor(mode2)#%d  wall time: %lf", pid,
...
template <typename T> void *thread_consumer(void *arg) {
  ...
  double cpu_time = 0, pre_cpu = 0;
  parallel_framework_semaphore_t<T> *p_frame =
      (parallel_framework_semaphore_t<T> *)arg;
  // const int queue_size=p_frame->queue_size;
  ...
  for (;;) {
    ...
    assert(p_frame->item_status[p_frame->head] !=
           parallel_framework_semaphore_t<T>::item_Waiting);
  ...
  ul_writelog(UL_LOG_NOTICE, "consumer CPU time: %lf, wall time: %lf", cpu_time,
              get_wall_time() - wall_time);
  return (void *)0;
}
template <typename T> int parallel_framework_semaphore_t<T>::create_threads() {
  ...
  for (int i = 0; i < num_processor; i++) {
    ...
    if (_mode != 2) {
      ...
      if (0 > pthread_create(_processor_tid + i, NULL,
                             thread_processor_mode2<T>,
                             (void *)(processor_arg + i))) {
        ul_writelog(UL_LOG_FATAL, "Create processor thread #%d failed", i);
        status = frame_Ready;
        return -1;
  ...
  ul_writelog(UL_LOG_NOTICE, "%d processors [mode:%d] threads are created",
              num_processor, _mode);
  if (_mode == 1) {
...
inline int parallel_framework_semaphore_t<T>::consume_one_item() {
  ...
  if (st) {
    ul_writelog(UL_LOG_WARNING, "consumer failed to consume one item at "
                                "[head:%d] [return:%d].The item is skipped.",
                head, st);
  } else {
    consumer_item_count++;
  }
...
template <typename T>
inline int parallel_framework_semaphore_t<T>::process_one_item(int pos,
                                                               int pid) {
  int pres;
  if ((pres = _processor(array[pos], pid)) == 0) {
    ...
    //    ul_writelog(UL_LOG_NOTICE,"processor #%d snap head=%d tail=%d pos=%d
    //    size=%d
    //    rem=%d",pid,head,tail,pos,(tail+queue_size-head)%queue_size,(tail+queue_size-pos)%queue_size);
    //  }
  } else {
...
template <typename T>
int parallel_framework_semaphore_t<T>::run(functionT generator,
                                           functionT processor,
                                           functionC consumer, int mode) {
  if (mode < 0 || mode > 2) {
  ...
  if (status != frame_Ready) {
    ul_writelog(UL_LOG_FATAL, "Call parallel_framework_semaphore_t::create "
                              "before calling parallel_framework_t::run");
    return -1;
  }
  _mode = mode;
...
template <typename T> int parallel_framework_semaphore_t<T>::run1() {
...
generator_end:
  ...
  ul_writelog(UL_LOG_NOTICE,
              ...
              generator_item_count);
  ul_writelog(UL_LOG_NOTICE,
              "parallel_framework_semaphore_t::run for [mode:%d] exit!", _mode);

  ul_writelog(UL_LOG_NOTICE,
              "generator and consumer CPU time: %lf, wall time: %lf", cpu_time,
              get_wall_time() - wall_time);
  return 0;
}
template <typename T> int parallel_framework_semaphore_t<T>::run2() {
  // generator return:  0:succeed, 1:stop, <0:error, exit
  // Start generator
  const double wall_time = get_wall_time();
  double cpu_time = 0, pre_cpu = 0;
  ...
  ul_writelog(UL_LOG_NOTICE, "generator CPU time: %lf, wall time: %lf",
              cpu_time, get_wall_time() - wall_time);
  status = frame_Ready;
  void *thread_status;
  for (int i = 0; i < num_processor; i++) {
...
template <typename T> int parallel_framework_semaphore_t<T>::run0() {
  // generator return:  0:succeed, 1:stop, <0:error, exit
  // Start generator
  const double wall_time = get_wall_time();
  double cpu_time = 0, pre_cpu = 0;
...
#ifdef OUTPUT
    ul_writelog(UL_LOG_NOTICE, "generator generated item #%d head=%d tail=%d",
                generator_item_count, head, tail);
#endif
    item_status[tail] = item_Waiting;
    int pid = tail % num_processor;
    tail = ((tail + 1) == queue_size) ? 0 : tail + 1;
